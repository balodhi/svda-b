...
@click.option('--category', type=click.Choice([
    'atod', 'atow', 'dtoa', 'dtow', 'wtoa', 'wtod', 'ftos', 'ctof', 'shtoc', 'shtof', 'shtos'
...
@click.option('--epoch_size', type=str, default='source',
...
def experiment(category, arch, rnd_init, img_size, confidence_thresh, teacher_alpha, unsup_weight,
    ...
    from imageDA import ship_dataset
    ...
    with torch.cuda.device(gpu_id):
        ...
        if category == 'visda_train_val':
            ...
            d_source = ship_dataset.ShipCCDDataset(img_size=img_shape, range01=True, rgb_order=True)
            d_target = ship_dataset.ShipFLIRDataset(img_size=img_shape, range01=True, rgb_order=True)
            if adamix:
                d_src2tgt = office_dataset.OfficeWebcam2DslrDataset(img_size=img_shape, range01=True, rgb_order=True)
        elif category == 'ctos':
            d_source = ship_dataset.ShipCCDDataset(img_size=img_shape, range01=True, rgb_order=True)
            d_target = ship_dataset.ShipSWIRDataset(img_size=img_shape, range01=True, rgb_order=True)
            if adamix:
                d_src2tgt = office_dataset.OfficeWebcam2AmazonDataset(img_size=img_shape, range01=True, rgb_order=True)
        elif category == 'ftoc':
            d_source = ship_dataset.ShipFLIRDataset(img_size=img_shape, range01=True, rgb_order=True)
            d_target = ship_dataset.ShipCCDDataset(img_size=img_shape, range01=True, rgb_order=True)
            if adamix:
                d_src2tgt = office_dataset.OfficeWebcam2AmazonDataset(img_size=img_shape, range01=True, rgb_order=True)
        elif category == 'ftos':
            d_source = ship_dataset.ShipFLIRDataset(img_size=img_shape, range01=True, rgb_order=True)
            ...
            d_source = ship_dataset.ShipSWIRDataset(img_size=img_shape, range01=True, rgb_order=True)
            d_target = ship_dataset.ShipCCDDataset(img_size=img_shape, range01=True, rgb_order=True)
            if adamix:
                d_src2tgt = office_dataset.OfficeWebcam2AmazonDataset(img_size=img_shape, range01=True, rgb_order=True)
        elif category == 'stof':
            ...
            d_source = ship_dataset.ShipSpotting(img_size=img_shape, range01=True, rgb_order=True)
            d_target = ship_dataset.ShipCCDDataset(img_size=img_shape, range01=True, rgb_order=True)
            if adamix:
                d_src2tgt = ship_dataset.ShipSpotting2CCD(img_size=img_shape, range01=True, rgb_order=True)
        elif category == 'shtof':
            d_source = ship_dataset.ShipSpotting(img_size=img_shape, range01=True, rgb_order=True)
            d_target = ship_dataset.ShipFLIRDataset(img_size=img_shape, range01=True, rgb_order=True)
            if adamix:
                d_src2tgt = ship_dataset.ShipSpotting2flir(img_size=img_shape, range01=True, rgb_order=True)
        elif category == 'shtos':
            d_source = ship_dataset.ShipSpotting(img_size=img_shape, range01=True, rgb_order=True)
            d_target = ship_dataset.ShipSWIRDataset(img_size=img_shape, range01=True, rgb_order=True)
            if adamix:
        ...
        def f_pred_tgt_mult(X_sup):
            ...
            for aug_i in range(len(X_sup)):
                ...
                y_pred = F.softmax(teacher_net(X_var)[0], dim=1).data.cpu().numpy() if adamix else F.softmax(teacher_net(X_var)[0], dim=1).data.cpu().numpy()
        ...
        if adamix:
            # ------------------------------------------------------------------------
            if samp == True:
                 sampler = sampling.WeightedSampler.class_balancing_sampler(y=d_source.y)
            else:
                 sampler = None
            sup_mix_ds = data_source.ArrayDataSource([d_source.images, d_src2tgt.images, d_source.y], repeats=repeats, indices=source_indices, sampler=sampler)
            ...
            if samp == True:
                 ...
                 sampler = None
            sup_ds = data_source.ArrayDataSource([d_source.images, d_source.y], repeats=repeats, indices=source_indices, sampler=sampler)
            tgt_train_ds = data_source.ArrayDataSource([d_target.images], repeats=-1, indices=target_indices)
